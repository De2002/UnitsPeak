...
const AreaPairConverter = () => {
  const { pair } = useParams<{ pair: string }>();
  const navigate = useNavigate();

  const parsed = pair ? parseAreaPairSlug(pair) : null;
  const fromId = parsed?.fromId ?? "ac";
  const toId = parsed?.toId ?? "m2";
  ...
  useEffect(() => {
    if (!fromUnit || !toUnit) {
      navigate("/converter/area", { replace: true });
    }
  ...
  const handlePairChange = (from: string, to: string) => {
    setCurrentFrom(from);
    setCurrentTo(to);
    navigate(`/converter/area/${buildAreaPairSlug(from, to)}`, { replace: true });
  };
  ...
  const sampleValue = 1;
  const sampleResult = fromUnit && toUnit
    ? formatAreaResult(convertArea(sampleValue, fromId, toId))
    : "";

  useSeoMeta({
    title: `${fromName} to ${toName} Converter – Area Conversion | UnitsPeak`,
    description: `Instantly convert ${fromName} (${fromUnit?.symbol}) to ${toName} (${toUnit?.symbol}). Free, accurate area converter with reference table. ${sampleValue} ${fromUnit?.symbol} = ${sampleResult} ${toUnit?.symbol}.`,
    canonical: `https://www.unitspeak.com/converter/area/${pair}`,
    keywords: `${fromName.toLowerCase()} to ${toName.toLowerCase()}, ${fromUnit?.symbol} to ${toUnit?.symbol}, area converter, ${fromName.toLowerCase()} converter, unit conversion`,
  });

  if (!fromUnit || !toUnit) return null;

  const tableValues = [0.001, 0.01, 0.1, 0.25, 0.5, 1, 2, 5, 10, 50, 100, 500, 1000];
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1">
        ...
        <div className="border-b border-border/40 bg-gradient-to-b from-primary/5 to-transparent">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-5">
            <nav className="flex items-center gap-1.5 text-xs text-muted-foreground mb-2 flex-wrap" aria-label="Breadcrumb">
              ...
              <Link to="/converter/area" className="hover:text-primary transition-colors">Area</Link>
              <ChevronRight className="w-3 h-3" />
        ...
        {/* Convert from this unit to all other area units */}
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-10">
          <div className="flex items-center gap-3 mb-5">
            <h2 className="text-lg font-bold font-['Space_Grotesk'] text-foreground">
              Convert {fromName} to Other Area Units
            </h2>
            <div className="flex-1 h-px bg-gradient-to-r from-primary/20 to-transparent" />
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
            {AREA_UNITS.filter((u) => u.id !== fromId).map((target) => (
              <Link
                key={target.id}
                to={`/converter/area/${buildAreaPairSlug(fromId, target.id)}`}
            ...
            {POPULAR_AREA_PAIRS.filter(
              ...
              .map(({ from, to }) => {
                ...
                return (
                  <Link
                    key={`${from}-${to}`}
                    to={`/converter/area/${buildAreaPairSlug(from, to)}`}
...
export default AreaPairConverter;
