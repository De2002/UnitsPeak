...
import {
  ...
  convertVolume,
  formatVolumeResult,
  VOLUME_UNITS_BY_GROUP,
} from "@/constants/volumeUnits";

const QUICK_REF_TARGETS = ["m3", "L", "mL", "gal-us", "qt-us", "pt-us", "cup-us", "floz-us", "tbsp-us", "tsp-us"];

const VolumePairConverter = () => {
  const { pair } = useParams<{ pair: string }>();
  const navigate = useNavigate();

  const parsed = pair ? parseVolumePairSlug(pair) : null;
  const fromId = parsed?.fromId ?? "L";
  const toId = parsed?.toId ?? "gal-us";

  const fromUnit = VOLUME_UNIT_MAP[fromId];
  const toUnit = VOLUME_UNIT_MAP[toId];

  useEffect(() => {
    if (!fromUnit || !toUnit) {
      navigate("/converter/volume", { replace: true });
    }
  ...
  const handlePairChange = (from: string, to: string) => {
    setCurrentFrom(from);
    setCurrentTo(to);
    navigate(`/converter/volume/${buildVolumePairSlug(from, to)}`, { replace: true });
  };
  ...
  useSeoMeta({
    title: `${fromName} to ${toName} Converter – Volume Conversion | UnitsPeak`,
    description: `Instantly convert ${fromName} (${fromUnit?.symbol}) to ${toName} (${toUnit?.symbol}) online. Free, accurate, and real-time volume conversion tool. 1 ${fromUnit?.symbol} = ${formatVolumeResult(convertVolume(1, fromId, toId))} ${toUnit?.symbol}.`,
    canonical: `https://www.unitspeak.com/converter/volume/${pair}`,
    keywords: `${fromName.toLowerCase()} to ${toName.toLowerCase()}, ${fromUnit?.symbol} to ${toUnit?.symbol}, volume converter, liquid converter, unit conversion`,
  });
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1">
        ...
        <div className="border-b border-border/40 bg-gradient-to-b from-primary/5 to-transparent">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-5">
            <nav className="flex items-center gap-1.5 text-xs text-muted-foreground mb-2 flex-wrap" aria-label="Breadcrumb">
              ...
              <Link to="/converter/volume" className="hover:text-primary transition-colors">Volume</Link>
              <ChevronRight className="w-3 h-3" />
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex items-center justify-between mb-4">
            ...
            <ShareButton
              ...
              text={`Convert ${fromName} to ${toName} instantly. 1 ${fromUnit.symbol} = ${formatVolumeResult(convertVolume(1, fromId, toId))} ${toUnit.symbol}`}
            />
          </div>
                  ...
                  {conversionTableValues.map((v) => (
                    <tr key={v} className="hover:bg-muted/20 transition-colors">
                      ...
                      <td className="px-5 py-2.5 text-right text-primary font-semibold font-mono">
                        {formatVolumeResult(convertVolume(v, fromId, toId))} {toUnit.symbol}
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-10">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2.5">
            {QUICK_REF_TARGETS.filter((id) => id !== fromId).map((targetId) => {
              const targetUnit = VOLUME_UNIT_MAP[targetId];
              if (!targetUnit) return null;
              const result = convertVolume(1, fromId, targetId);
              return (
                <Link
                  key={targetId}
                  to={`/converter/volume/${buildVolumePairSlug(fromId, targetId)}`}
                ...
                >
                  ...
                  <p className="text-sm font-semibold text-primary font-mono truncate">
                    {formatVolumeResult(result)} {targetUnit.symbol}
                  </p>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-10">
          <div className="flex items-center gap-3 mb-5">
            <h2 className="text-lg font-bold font-['Space_Grotesk'] text-foreground">
              Convert {fromName} to Other Volume Units
            </h2>
            <div className="flex-1 h-px bg-gradient-to-r from-primary/20 to-transparent" />
          </div>

          <div className="space-y-6">
            {Object.entries(VOLUME_UNITS_BY_GROUP).map(([group, units]) => {
                ...
                <div key={group}>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
                    {targets.map((target) => (
                      <Link
                        key={target.id}
                        to={`/converter/volume/${buildVolumePairSlug(fromId, target.id)}`}
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2.5">
            {POPULAR_VOLUME_PAIRS.filter(
              (p) => !(p.from === fromId && p.to === toId) && !(p.from === toId && p.to === fromId)
            )
              .slice(0, 8)
              .map(({ from, to }) => {
                const f = VOLUME_UNIT_MAP[from];
                const t = VOLUME_UNIT_MAP[to];
                if (!f || !t) return null;
                return (
                  <Link
                    key={`${from}-${to}`}
                    to={`/converter/volume/${buildVolumePairSlug(from, to)}`}
                    className="glass-card group rounded-xl px-4 py-3 transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/40 block"
                  >
...
export default VolumePairConverter;
