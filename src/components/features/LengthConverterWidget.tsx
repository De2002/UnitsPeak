...
interface LengthConverterWidgetProps {
  initialFrom?: string;
  initialTo?: string;
  onPairChange?: (fromId: string, toId: string) => void;
}

const LengthConverterWidget = ({
  initialFrom = "m",
  initialTo = "ft",
  onPairChange,
}: LengthConverterWidgetProps) => {
  const [fromId, setFromId] = useState(initialFrom);
  const [toId, setToId] = useState(initialTo);
  const [fromValue, setFromValue] = useState("1");
  const [toValue, setToValue] = useState("");

  // Sync when props change (e.g. navigating to a pair page)
  useEffect(() => {
    setFromId(initialFrom);
    setToId(initialTo);
  }, [initialFrom, initialTo]);

  // Recompute output whenever from value or units change
  useEffect(() => {
    const parsed = parseFloat(fromValue);
    if (!isNaN(parsed) && fromValue.trim() !== "") {
      setToValue(formatLengthResult(convertLength(parsed, fromId, toId)));
    } else {
      setToValue("");
    }
    ...
    <div className="py-2">
      ...
      <div className="space-y-2">
        ...
        <UnitCombobox
          value={fromId}
          units={LENGTH_UNITS}
          unitsByGroup={LENGTH_UNITS_BY_GROUP}
          onChange={handleFromUnitChange}
          exclude={toId}
          placeholder="Search length units…"
        />
      </div>
      ...
      {fromUnit && toUnit && fromValue && toValue && (
        <div className="mt-5 rounded-2xl bg-primary/8 border border-primary/20 px-5 py-5 sm:px-7 sm:py-6 overflow-hidden">
          ...
          {/* Result value + unit — large & prominent */}
          <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5 min-w-0">
          ...
          <p className="text-xs text-muted-foreground/50 mt-3 font-mono break-all">
            1 {fromUnit.symbol} = {formatLengthResult(convertLength(1, fromId, toId))}{" "}
            {toUnit.symbol}&nbsp;·&nbsp;1 {toUnit.symbol} ={" "}
            {formatLengthResult(convertLength(1, toId, fromId))} {fromUnit.symbol}
          </p>
        </div>
      )}
    </div>
...
export default LengthConverterWidget;
