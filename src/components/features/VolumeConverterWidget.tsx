...
interface VolumeConverterWidgetProps {
  initialFrom?: string;
  initialTo?: string;
  onPairChange?: (fromId: string, toId: string) => void;
}

const VolumeConverterWidget = ({
  initialFrom = "L",
  initialTo = "gal-us",
  onPairChange,
}: VolumeConverterWidgetProps) => {
  ...
  useEffect(() => {
    ...
    if (!isNaN(parsed) && fromValue.trim() !== "") {
      setToValue(formatVolumeResult(convertVolume(parsed, fromId, toId)));
    } else {
      setToValue("");
    }
    ...
    <div className="py-2">
      ...
      <div className="space-y-2">
        ...
        <UnitCombobox
          value={fromId}
          units={VOLUME_UNITS}
          unitsByGroup={VOLUME_UNITS_BY_GROUP}
          onChange={handleFromUnitChange}
          exclude={toId}
          placeholder="Search volume units…"
        />
      </div>
      ...
      {fromUnit && toUnit && fromValue && toValue && (
        <div className="mt-5 rounded-2xl bg-primary/8 border border-primary/20 px-5 py-5 sm:px-7 sm:py-6 overflow-hidden">
          ...
          <p className="text-xs text-muted-foreground/50 mt-3 font-mono break-all">
            1 {fromUnit.symbol} = {formatVolumeResult(convertVolume(1, fromId, toId))}{" "}
            {toUnit.symbol}&nbsp;·&nbsp;1 {toUnit.symbol} ={" "}
            {formatVolumeResult(convertVolume(1, toId, fromId))} {fromUnit.symbol}
          </p>
        </div>
      )}
    </div>
...
export default VolumeConverterWidget;
